...
import mail.api.mail.IPostRegistry;
...
public class PostRegistry implements IPostRegistry {
    ...
    public boolean isValidPOBox(World world, IMailAddress address) {
        Log.info("This world is a client: vor valid ceck" + world.isRemote);
        return address.getType() == EnumAddressee.PLAYER && address.getName().matches("^[a-zA-Z0-9]+$");
    }

    @Nullable
    public  POBox getPOBox(World world, IMailAddress address) {
    ...
    public  POBox getOrCreatePOBox(World world, IMailAddress address) {
