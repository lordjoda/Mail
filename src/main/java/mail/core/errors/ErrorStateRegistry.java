...
import mail.api.core.IErrorStateRegistry;
...
public class ErrorStateRegistry implements IErrorStateRegistry {
	...
	public void registerErrorState(IErrorState state) {
		if (states.containsKey(state.getID())) {
			throw new RuntimeException("BungeeMail Error State does not possess a unique id.");
		}
	...
	public void addAlias(IErrorState state, String name) {
		if (!states.values().contains(state)) {
			throw new RuntimeException("BungeeMail Error State did not exist while trying to register alias.");
		}
	...
	private static void addStateName(IErrorState state, String name) {
		if (!name.contains(":")) {
			throw new RuntimeException("BungeeMail Error State name must be in the format <modid>:<name>.");
		}

		if (stateNames.containsKey(name)) {
			throw new RuntimeException("BungeeMail Error State does not possess a unique name.");
		}
